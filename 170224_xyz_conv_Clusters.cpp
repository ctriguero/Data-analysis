...
int main( int argc, const char* argv[] )
{
	...
	//std::stringstream name = argv[1];
	cout << name << endl ;
	name.erase (name.end()-4, name.end()) ;
	cout << name << endl ;
	std::string nameout ;
	nameout = name + "_Ang.xyz"  ;
	cout << nameout << endl ;	

	
	// Files to store data
	ofstream OUT ;
//	OUT.open ("Cluster3_Ang.xyz", ios::out | ios::trunc); // All computed densities inside cylinders
	OUT.open (nameout.c_str(), ios::out | ios::trunc); 
