...
//			cout << "[" << KLimitDown << " " << KLimitUp << "]" << endl ;
			...
			for ( int j = 0; j < Kbins; ++j )
			{
				if ( ( Frequency[i][j] > LimitDown ) && ( Frequency[i][j] <= LimitUp ) ) 
				{
					...
					if ( Control == 1 ) // To add this line only once and not repeat d1 d1 d1 in the loop
					{
						...
						GLE_DSK << "Layer = " << k+1 << std::endl ;
						GLE_DSK << "d[Layer] marker fsquare msize 0.11 color rgb((Layer-1)^0.5/(Layers-1)^0.5+(Layers-Layer-1)^10/(Layers-1)^10,(Layers-Layer-1)^3/(Layers-1)^3,(Layers-Layer-1)^5/(Layers-1)^5)" << std::endl ;
						GLE_DSK << std::endl ;
					}
				}
			}
	...
	GLE_DSK << "data Likely_SK_path.dat d[Layers+3]" << std::endl ;
	GLE_DSK << "d[Layer+3] line color seagreen lwidth 0.025 lstyle 0" << std::endl ;
	GLE_DSK << "data Expected_SK_path.dat d[Layers+4]=c1,c2" << std::endl ;
	...
	GLE_DSK << "\t" << "rr = (Layers*z)^0.5/(Layers-1)^0.5+(Layers-Layers*z)^10/(Layers-1)^10" << std::endl ;
	GLE_DSK << "\t" << "! GREEN" << std::endl ;
	GLE_DSK << "\t" << "gg = (Layers-Layers*z)^3/(Layers-1)^3" << std::endl ;
	...
	{	
		...
		for ( int ns = 1; ns < Sbins ; ++ns )
		{
			...
			if (   ns <= 9 ) { dd << "DKatS_no_000" << ns << "_free.dat" ;}
			if ( ( ns > 9 ) && ( ns <= 99 ) ) { dd << "DKatS_no_00" << ns << "_free.dat" ;}
			if ( ( ns > 99 ) && ( ns <= 999 ) ) { dd << "DKatS_no_0" << ns << "_free.dat" ;}
			if ( ( ns > 999 ) && ( ns <= 9999 ) ) { dd << "DKatS_no_" << ns << "_free.dat" ;}
