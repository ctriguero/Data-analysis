...
//			cout << "[" << KLimitDown << " " << KLimitUp << "]" << endl ;
			...
			for ( int j = 0; j < Kbins; ++j )
			{
				if ( ( Frequency[i][j] > LimitDown ) && ( Frequency[i][j] <= LimitUp ) ) 
				{
					...
					if ( Control == 1 ) // To add this line only once and not repeat d1 d1 d1 in the loop
					{
						...
						double zz = 1.0*k/(1.0*Layers) ;
						//double zz = 1.0-1.0*k/(1.0*Layers-1.0) ;//+0.342685 ;
	...
	GLE_DSK << "d[Layer+1] line color black lwidth 0.05 lstyle 9" << std::endl ;
	GLE_DSK << "let d[Layer+2] = Kmin(x) from 2.0 to 55" << std::endl ;
	GLE_DSK << "d[Layer+2] line color black lwidth 0.05 lstyle 0" << std::endl ;
...
//	GLE_DSK << "xaxis min 1.5 max 56  log format \"sci 0 10\"" << std::endl ;
	...
	GLE_DSK << "!pos br" << std::endl ;
	GLE_DSK << "!nobox" << std::endl ;
	GLE_DSK << "!text \"T-clustering Metadynamics - NPT\"" << std::endl ;
	GLE_DSK << "!text \"Histogram over $5.0\\cdot 10^5$ data points\"" << std::endl ;
	GLE_DSK << "!end key" << std::endl ;
	...
	GLE_DSK << "\t" << "local t = z" << std::endl ;
	GLE_DSK << "\t" << "local L1 = 0.07" << std::endl ;
	GLE_DSK << "\t" << "local L2 = 0.20" << std::endl ;
	GLE_DSK << "\t" << "local L3 = 0.35" << std::endl ;
	GLE_DSK << "\t" << "local L4 = 0.45" << std::endl ;
	GLE_DSK << "\t" << "local L5 = 1.00" << std::endl ;

	GLE_DSK << "\t" << "if (z > dom1) and (z <= dom2) then rr = 1" << std::endl ;

	GLE_DSK << "\t" << "if ( z <= L1 ) then r = 0" << std::endl ;
	GLE_DSK << "\t" << "if ( z <= L1 ) then g = 1/L1*t" << std::endl ;
	GLE_DSK << "\t" << "if ( z <= L1 ) then b = 1" << std::endl ;

	GLE_DSK << "\t" << "if ( z > L1 ) and ( z <= L2 ) then r = 0" << std::endl ;
	GLE_DSK << "\t" << "if ( z > L1 ) and ( z <= L2 ) then  g = 1" << std::endl ;
	GLE_DSK << "\t" << "if ( z > L1 ) and ( z <= L2 ) then  b = -1/(L2-L1)*t-L2/(L1-L2)" << std::endl ;

	GLE_DSK << "\t" << "if ( z > L2 ) and ( z <= L3 ) then  r = -1/(L2-L3)*t+L2/(L2-L3)" << std::endl ;
	GLE_DSK << "\t" << "if ( z > L2 ) and ( z <= L3 ) then  g = 1" << std::endl ;
	GLE_DSK << "\t" << "if ( z > L2 ) and ( z <= L3 ) then  b = 0" << std::endl ;

	GLE_DSK << "\t" << "if ( z > L3 ) and ( z <= L4 ) then  r = 1" << std::endl ;
	GLE_DSK << "\t" << "if ( z > L3 ) and ( z <= L4 ) then  g = -1/(L4-L3)*t-L4/(L3-L4)" << std::endl ;
	GLE_DSK << "\t" << "if ( z > L3 ) and ( z <= L4 ) then  b = 0" << std::endl ;

	GLE_DSK << "\t" << "if ( z > L4 ) and ( z <= L5 ) then  r = -1/(L5-L4)*t-L5/(L4-L5)" << std::endl ;
	GLE_DSK << "\t" << "if ( z > L4 ) and ( z <= L5 ) then  g = 0" << std::endl ;
	GLE_DSK << "\t" << "if ( z > L4 ) and ( z <= L5 ) then  b = 0" << std::endl ;

	GLE_DSK << "\t" << "return rgb(r,g,b)" << std::endl ;
	GLE_DSK << "end sub" << std::endl ;
	...
	{	
		...
		for ( int ns = 1; ns < Sbins ; ++ns )
		{
			...
			for ( int nk = 0; nk < Kbins ; ++nk ) { IntegralDKS = IntegralDKS + Frequency[ns][nk]*KInterval ; }
			...
			for ( int nk = 0; nk < Kbins ; ++nk )
			{
				...
				{
					K_PDF_at_S << nk*KInterval+KInterval/2.0 << "\t" << Frequency[ns][nk]/IntegralDKS << std::endl ;
					if ( ns > 2 )
					{
						SingleFileDKS << ns << "\t" << nk*KInterval+KInterval/2.0 << "\t" << Frequency[ns][nk]/IntegralDKS << std::endl ;
					}
				}


				if ( Frequency[ns][nk] > LikelyPathK ) { LikelyPathK = Frequency[ns][nk] ; } // Detecting the max value
			}
			K_PDF_at_S.close () ;
			...
			for ( int nk = 0; nk < Kbins ; ++nk ) 
			{
				...
				AVK = AVK + (nk*KInterval+KInterval/2.0)*Frequency[ns][nk]/IntegralDKS ; // <K>= \int_{Kmin}^{Kmax} K D(K|S) dK
				AVK2 = AVK2 + (nk*KInterval+KInterval/2.0)*(nk*KInterval+KInterval/2.0)*Frequency[ns][nk]/IntegralDKS ; // <K>= \int_{Kmin}^{Kmax} K^2 D(K|S) dK
			}
			...
			for ( int nk = 0; nk < Kbins ; ++nk ) 
			{
				STDK2 = STDK2 + pow((nk*KInterval+KInterval/2.0)-AVK,2.0)*Frequency[ns][nk]/IntegralDKS ;
			}
			STDK = sqrt(STDK2) ;
			// END Standard deviation
			
			EXPECTEDPATH << ns << "\t" << AVK*KInterval+KInterval/2.0 << "\t" << "Think norm" << "\t" << STDK << "\t" << AVK*KInterval+KInterval/2.0+STDK << "\t" << AVK*KInterval+KInterval/2.0-STDK << std::endl ; 
