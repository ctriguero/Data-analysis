...
//	cout << YELLOW << "    -> Detected " << BOLDYELLOW << KVector.size () << RESET << YELLOW << " k data points" << RESET << endl ;
	...
	{ 
		for ( int j = 0; j < Kbins+1; ++j ) 
		{
			...
			{ 
				Frequency[i][j] = log(HVector[i*(Kbins+1)+j]) ;
...
//	GLE_DSK << "!color_range_vertical zmin 0 zmax 1 zstep 0.25 pixels Layers palette carles format \"fix 2\"" << std::endl ;
	...
	GLE_DSK << "color_range_horizontal zmin " << MiN << " zmax " << MaX << " zstep " << (MaX-MiN)/4.0 << " pixels Layers palette carles format \"fix 4\"" << std::endl ;
...
//	GLE_DSK << "end key" << std::endl ;
	...
	GLE_DSK << "\t" << "local L1 = 0.25" << std::endl ;
	GLE_DSK << "\t" << "local L2 = 0.40" << std::endl ;
	GLE_DSK << "\t" << "local L3 = 0.50" << std::endl ;
	GLE_DSK << "\t" << "local L4 = 0.65" << std::endl ;
	...
	system("rm -f Layer_no_*.dat") ; // Erase anoying layer files
...
//				}
	...
	system("rm -f DKatS_no_*.dat") ; // Erase anoying layer files
