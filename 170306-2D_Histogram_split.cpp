	...
	for ( int k = 0; k < Layers; ++k )
	{
		...
		for ( int i = 0; i < Sbins; ++i )
		{
			for ( int j = 0; j < Kbins; ++j )
			{
				...
				{
					LayerFile << i*1.0 << "    " << j*KInterval+KInterval/2.0 << "    " << Frequency[i][j] << std::endl ; 
				}
			}
		}
...
//	// BEGIN D(K|S): Conditional probability for each size. Each size in one file.
//	for ( int S = 0; i < SVector.size () ; ++i )
//	{
...
//		}	
//	}
//	// END D(K|S): Conditional probability for each size. Each size in one file.
