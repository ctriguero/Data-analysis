...
using namespace std;
 ...
 int main()
	...
	std::ifstream InputFile2( "traj_all.xyz" ) ; // file where the extraction is performed
...
//				std::cout << element << " " << x << " " << y << " " << z << std::endl ;
				OutputFile << "sphere { <" << x << "," << y << "," << z << ">," << Rgas << " pigment { rgbt <0,0,1,0>} finish { phong 0.7 phong_size 20 }}\n";
			}
			OutputFile.close () ;
		}
	}
